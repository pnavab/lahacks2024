                    ...
                    <div className='mt-[100px] w-full flex flex-col items-center mb-auto'>
                        {!joinedLobby &&
                             <div className='pb-4 flex flex-row'>
                                 <input placeholder='username' className='px-5 py-2 bg-indigo-200  text-black rounded-l-md border-white border-2 w-40' onChange={(e) => setUsername(e.target.value)}></input>
                                <button className='bg-[#4f80e2] text-white px-5 py-2 duration-200 rounded-r-md hover:bg-[#3e54d3] border-2' onClick={joinCollaborativeCanvas}>Join Lobby</button>
                            </div>
                        }
                        <button className='bg-[#4f80e2]  text-white px-5 py-2 mb-3 rounded-md duration-200 hover:bg-[#3e54d3] border-2' onClick={copyUrlToClipboard} >Copy Invite Link</button>
                    {joinedLobby ?
                        <div className="grid grid-cols-1 ">
                            <h1 className='text-center w-full px-4 py-2 bg-indigo-300 rounded-t-md'>Players:</h1>
                            {connectedUsers.map((user, index) => (
                                <div key={index} className={`text-black bg-indigo-300 ${index === connectedUsers.length - 1 ? 'rounded-b-md' : ''} w-full`}>
                                        ...
                                        <div className="relative w-auto my-6 mx-auto max-w-3xl">
                                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                                ...
                                                <div className="flex items-center justify-end p-6 border-t border-solid border-gray-300 rounded-b">
                                                    ...
                                                    <button
                                                        className="bg-green-400 text-white active:bg-green-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1"
                                                        type="button"
                                                        style={{ transition: "all .15s ease" }}
                                                        onClick={() => setShowSettingsModal(false)}
                                                    >
