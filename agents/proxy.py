...
async def validator_agent_query(req):
    response = await query(destination=VALIDATOR_AGENT, message=req, timeout=15.0)
    print(response)
    data = json.loads(response.decode_payload())
    return data
...
async def make_agent_call(req: Request):
    try:
        ...
        validatorRes = await validator_agent_query({"correct":req.correct,"guessed":guessingRes.guessed})
